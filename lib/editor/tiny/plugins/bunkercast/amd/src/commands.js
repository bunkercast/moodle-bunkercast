 ...
 *
 * @module     tiny_bunkercast/commands
...
const openPicker = async (editor, strings) => {
    ...
    editor.windowManager.open({
        ...
        onSubmit: async (dialog) => {
            ...
            // Authorise the video for this course BEFORE inserting it. The filter
            // refuses to mint a playback link for a video that was never
            // authorised where it is shown, so inserting first would leave a
            // placeholder that renders a dead player — worse than inserting
            // nothing, because the failure surfaces later and somewhere else.
            try {
                await Ajax.call([{
                    methodname: 'filter_bunkercast_register_embed',
                    args: {fileid, contextid},
                }])[0];
            } catch (e) {
                editor.windowManager.alert(strings.registerfailed);
                window.console.warn('tiny_bunkercast: could not authorise video', e);
                return;
            }

            // The same placeholder a teacher could type by hand. The filter
            // resolves it to a player at view time; nothing about the video's
            // protection depends on this plugin.
